...
export class WorkflowsSurrealDB extends WorkflowsStorage {
	...
	async updateWorkflowResults({
		...
		Record<string, StepResult<unknown, unknown, unknown, unknown>>
	> {
		const current = await this.loadWorkflowSnapshot({
			workflowName,
			runId,
		});
		if (!current)
			throw new Error(
				`Workflow snapshot not found: ${workflowName}/${runId}`,
			);

		const updatedResults = {
			...((
				current as WorkflowRunState & {
       ...
       WHERE workflowName = $wn AND runId = $rid`,
		...
		return updatedResults as Record<
			string,
	...
	async updateWorkflowState({
		...
		const merged: WorkflowRunState = {
			...
			...(opts.suspendedPaths !== undefined
				? { suspendedPaths: opts.suspendedPaths }
				: {}),
			...(opts.waitingPaths !== undefined
	...
	async getWorkflowRunById({
		...
		if (workflowName) {
			surql =
				'SELECT * FROM mastra_workflow_snapshot WHERE runId = $runId AND workflowName = $wn LIMIT 1';
			bindings.wn = workflowName;
		} else {
			surql =
				'SELECT * FROM mastra_workflow_snapshot WHERE runId = $runId LIMIT 1';
		}
	...
	async listWorkflowRuns(
		...
		const limitClause = paginate
			? `LIMIT ${normalizedPerPage} START ${offset}`
			: '';
