...
export class SurrealDBClient {
  ...
  async queryAll<T>(surql: string, bindings?: Record<string, unknown>): Promise<T[]> {
    const results = await this.db.query<[T[]]>(surql, bindings);
    return (results[0] as T[]) ?? [];
  }

  async queryOne<T>(surql: string, bindings?: Record<string, unknown>): Promise<T | null> {
    const rows = await this.queryAll<T>(surql, bindings);
    return rows[0] ?? null;
  }

  async queryCount(surql: string, bindings?: Record<string, unknown>): Promise<number> {
    const rows = await this.db.query<[Array<{ count: number }>]>(surql, bindings);
    return rows[0]?.[0]?.count ?? 0;
  }

  async execute(surql: string, bindings?: Record<string, unknown>): Promise<void> {
