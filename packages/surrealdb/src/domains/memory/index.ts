...
export class MemorySurrealDB extends MemoryStorage {
  ...
  async listThreads(args: StorageListThreadsInput): Promise<StorageListThreadsOutput> {
    ...
    const normalizedPerPage = perPage === false ? Number.MAX_SAFE_INTEGER : (perPage ?? 100);
    const offset = page * normalizedPerPage;

    const { field: sortField, direction: sortDir } = this.parseOrderBy(orderBy);
    const orderClause = `ORDER BY ${sortField} ${sortDir.toUpperCase()}`;

    let whereClause = '1 = 1';
    const bindings: Record<string, unknown> = { limit: normalizedPerPage, offset };
  ...
  async listMessages(args: StorageListMessagesInput): Promise<StorageListMessagesOutput> {
    const { threadId, resourceId, page = 0, perPage = 40, filter, orderBy } = args;
    ...
    const { field: sortField, direction: sortDir } = this.parseOrderBy(orderBy);
    const normalizedPerPage = perPage === false ? Number.MAX_SAFE_INTEGER : (perPage ?? 40);
    const offset = page * normalizedPerPage;
  ...
  async saveMessages({ messages }: { messages: MastraDBMessage[] }): Promise<{ messages: MastraDBMessage[] }> {
    ...
    await this.client.tx(async (db) => {
      for (const msg of messages) {
        ...
        await db.query(`UPSERT type::thing('mastra_messages', $id) CONTENT $data`, {
          id: msg.id,
          data,
        });
      }
  ...
  async updateMessages(
    ...
    for (const patch of args.messages) {
      ...
      if (patch.resourceId !== undefined) merge['resourceId'] = patch.resourceId;

      if (patch.content) {
        if (patch.content.metadata !== undefined) merge['content.metadata'] = patch.content.metadata;
        if (patch.content.content !== undefined) merge['content.content'] = patch.content.content;
      }
  ...
  override async updateResource({
    ...
    if (!updated) throw new Error(`Resource not found after update: ${resourceId}`);
    return updated;
  }
}
