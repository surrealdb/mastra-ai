...
): { listSurql: string; countSurql: string; listBindings: Record<string, unknown> } {
...
export class ScoresSurrealDB extends ScoresStorage {
  ...
  async saveScore(score: SaveScorePayload): Promise<{ score: ScoreRowData }> {
    const id = (score as Record<string, unknown>)['id'] as string | undefined ?? randomUUID();
    const now = new Date();
  ...
  async listScoresByScorerId({
    ...
    if (entityId) { whereParts.push('entityId = $entityId'); bindings['entityId'] = entityId; }
    if (entityType) { whereParts.push('entityType = $entityType'); bindings['entityType'] = entityType; }
    if (source) { whereParts.push('source = $source'); bindings['source'] = source; }

    const { listSurql, countSurql, listBindings } = buildPaginated('mastra_scorers', whereParts.join(' AND '), bindings, pagination);
    return paginateScores(this.client, listSurql, countSurql, listBindings, bindings, pagination);
  }
  ...
  async listScoresByRunId({
    ...
    const { listSurql, countSurql, listBindings } = buildPaginated('mastra_scorers', 'runId = $runId', bindings, pagination);
    return paginateScores(this.client, listSurql, countSurql, listBindings, bindings, pagination);
  }
  ...
  async listScoresByEntityId({
    ...
    const { listSurql, countSurql, listBindings } = buildPaginated('mastra_scorers', 'entityId = $entityId AND entityType = $entityType', bindings, pagination);
    return paginateScores(this.client, listSurql, countSurql, listBindings, bindings, pagination);
  }
}
