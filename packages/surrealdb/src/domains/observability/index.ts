...
export class ObservabilitySurrealDB extends ObservabilityStorage {
  ...
  override get tracingStrategy() {
    return {
      ...
      supported: ['insert-only'] as Array<'realtime' | 'batch-with-updates' | 'insert-only' | 'event-sourced'>,
    };
  }
  ...
  override async batchCreateSpans({ records }: BatchCreateSpansArgs): Promise<void> {
    ...
    await this.client.execute(
      'INSERT INTO mastra_ai_spans $rows',
      { rows },
    );
  }

  override async getSpan({ spanId, traceId }: GetSpanArgs): Promise<GetSpanResponse | null> {
  ...
  override async getTrace({ traceId }: GetTraceArgs): Promise<GetTraceResponse | null> {
  ...
  override async listTraces(args: ListTracesArgs): Promise<ListTracesResponse> {
    ...
    const perPage: number | false = (perPageRaw as number | false | undefined) ?? 100;
    ...
    const normalizedPerPage: number = perPage === false ? Number.MAX_SAFE_INTEGER : (perPage ?? 100);
